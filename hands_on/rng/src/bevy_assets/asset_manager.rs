...
impl AssetManager {
    ...
    pub fn add_image<S: ToString>(mut self, tag: S, filename: S) -> Result<Self> {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            if !new_image.exists() {
                return Err(Error::msg(format!("{} not found in assets directory", &filename)));
            }
        }

        self.asset_list.push((tag.to_string(), filename, AssetType::Image));
...
pub(crate) fn setup_asset_store(
    ...
    asset_resource.asset_list.iter().for_each(
        |(tag, filename, asset_type)| {
            match asset_type {
                _ => {
                    assets.asset_index.insert(tag.clone(), asset_server.load_untyped(filename));
                }
            }
        });
