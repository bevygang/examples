...
use crate::{AssetStore, AssetManager};
use crate::flow::menus::MenuResource;
use crate::egui::egui::Window;
...
pub(crate) fn setup(
    ...
    let assets = match assets {
        ...
        None => &setup_asset_store(asset_manager.as_ref().unwrap(), &mut commands, &asset_server),
    };

    let assets_to_load: Vec<Handle<LoadedUntypedAsset>> = assets.asset_index.values().cloned().collect();
