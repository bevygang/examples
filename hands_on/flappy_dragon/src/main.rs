...
fn setup(
  ...
  build_wall(&mut commands, assets.wall.clone(), rng.random_range(-5..5));//(12)
...
fn gravity(mut query: Query<(&mut Flappy, &mut Transform)>) {
  if let Ok((mut flappy, mut transform)) = query.single_mut() {//(16)
...
fn move_walls(
  ...
  if rebuild {
    ...
    build_wall(&mut commands, assets.wall.clone(), rng.random_range(-5..5));
  }
}
...
fn hit_wall(
  ...
  if let Ok(player) = player.single() {//(26)
    for wall in walls.iter() {//(27)
